...
# venv 에는 finetune + unsloth 그룹이 함께 설치되어 있으므로 항상 두 그룹을 지정한다 (한 그룹만 주면 uv 가 나머지를 지운다).
set -euo pipefail
...
cd "$REPO"
uv run --group finetune --group unsloth python scripts/vram_probe.py --model "$MODEL" --seq-len "$SEQ" --batch "$BATCH" --env wsl2 "$@" 2>&1 \
